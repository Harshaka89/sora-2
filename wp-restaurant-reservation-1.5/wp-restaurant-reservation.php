...
/**
 * ✅ MAIN PLUGIN CLASS (PROPERLY CLOSED)
 */
class YenolxRestaurantReservation {
    ...
    private function init_frontend() {
        ...
        $frontend_file = YRR_PLUGIN_PATH . 'controllers/class-frontend-controller.php';
        if (file_exists($frontend_file)) {
            require_once $frontend_file;
            if (class_exists('YRR_Frontend_Controller')) {
                $frontend_controller = new YRR_Frontend_Controller();
                
                // Register shortcode
                add_shortcode('yenolx_booking_form', array($frontend_controller, 'display_booking_form'));
            }
        }
    }
    ...
    public function activate() {
        ...
        wp_cache_flush();
        delete_transient('yrr_db_check_done_v151');
